...
import markercluster from "../../../node_modules/leaflet.markercluster/dist/leaflet.markercluster";
...
export default class LoadLayer {
  ...
  public loadClusterLayer = () => {
    MapInt.removeLayerByid("clusetrId");
    let clusterPic = require("../../assets/良.png");
    console.log(markercluster.MarkerClusterGroup());
    let markClusters = markercluster
      .MarkerClusterGroup({
    ...
    esri
      ...
      .run(function(error, featureCollection) {
        featureCollection.features.forEach((ele) => {
          ...
          markClusters.push(marks);
        });
      });
    //MapInt.setLayerInfo("clusetrId", );
  };
}
