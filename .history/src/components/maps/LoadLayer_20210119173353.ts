...
export default class LoadLayer {
  ...
  public staticLayer = () => {
    ...
    if (MapInt.getInstance().hasLayer(plableLayer)) {
  ...
  public loadEchartLayer = () => {
    let pGroupLayer = L.layerGroup().addTo(MapInt.getInstance());
    esri
      .featureLayer({
        url: mapConfig.cszxd,
      })
      ...
      .run(function(error, featureCollection) {
        if (featureCollection.features) {
          featureCollection.features.forEach((ele) => {
            let echartId: any = ele.properties.FID;
            let html =
              "<div id=echart" +
              ele.properties.FID +
              " style='width:90px;height:90px;position:relative;background-color:red;'><div>";
